...
class ConversionService:
    ...
    async def list_for_affiliate(
        ...
        *,
        ...
        return await self.conversion_repo.list_by_affiliate(
            affiliate.id,
            skip=skip,
            limit=limit,
        )

    async def list_all(self, user: User, *, skip: int = 0, limit: int = 100) -> list[Conversion]:
        if user.role != UserRole.ADMIN:
            raise ForbiddenError("Only admins can list all conversions")
        return await self.conversion_repo.list_all(skip=skip, limit=limit)
