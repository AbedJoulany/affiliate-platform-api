...
import type { DiscoveryParams, DiscoveryProduct } from "../types/api";
import { DiscoveryFilterPanel, validateDiscoveryDraft } from "./DiscoveryFilterPanel";
...
export function DiscoveryView() {
  ...
  const selection = useDiscoverySelection(items);
  ...
  const handleExport = () => {
    const rows = selection.selectedProducts.length > 0 ? selection.selectedProducts : items;
  ...
  return (
    <PageContainer>
      <DiscoveryHeader
        ...
        lastRunStatus={session.lastRunStatus === "running" && discovery.isFetching ? "running" : session.lastRunStatus}
      ...
      <div className="mt-4">
        <DiscoveryStats
          ...
          lastRunStatus={
            discovery.isFetching ? "running" : session.lastRunStatus
          }
      ...
      <div className="mt-4 space-y-3">
        <DiscoveryIntentTabs
          ...
          onChange={(mode) => updateDraft({ mode, category_id: mode === "category" ? draft.category_id : undefined, page: 1 })}
        />
      </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        <DiscoveryFilterPanel
          ...
          categoryError={draft.mode === "category" ? draftError ?? undefined : undefined}
          onChange={updateDraft}
        />

        <section className="min-w-0 space-y-4" aria-label="نتائج الاكتشاف">
          {!canImport && (
          ...
          <DiscoveryResultsToolbar response={response} onPageChange={onPageChange} />

          {showInitialEmpty ? (
            <EmptyState
              title="لا توجد جلسة اكتشاف بعد"
              description="اضبط المصدر والفلاتر، ثم اضغط «تشغيل الاكتشاف» لبدء مساحة العمل."
            />
            ...
            <EmptyState title="لا توجد نتائج" description="وسّع الفلاتر أو غيّر المصدر ثم أعد التشغيل." />
          ) : (
            <DiscoveryResultsTable
              ...
              onScoreClick={(product) => setInspectorId(product.aliexpress_product_id)}
      ...
      {inspectorProduct && (
        <div className="fixed inset-0 z-40 bg-black/40" onClick={() => setInspectorId(null)} aria-hidden />
      )}
